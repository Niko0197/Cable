...
app.post('/calculate', (req, res) => {
    const { series, technology, distanceAts, customParams, entrances: reqEntrances, floors: reqFloors, entryPoint } = req.body;
    ...
    // Приоритет параметрам из запроса (подъезды и этажи)
    if (reqEntrances !== undefined && reqEntrances !== null && !isNaN(reqEntrances) && reqEntrances > 0) {
        bParams.entrances = Number(reqEntrances);
    }
    if (reqFloors !== undefined && reqFloors !== null && !isNaN(reqFloors) && reqFloors > 0) {
        bParams.floors = Number(reqFloors);
    }
    ...
    res.json({
        building: {
            totalSubscribers,
            entryPoint: entryPoint || 'basement',
            ...bParams
        },
        technology,
        estimates: {
